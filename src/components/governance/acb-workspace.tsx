...
import { AlertTriangle, CheckCircle, Clock, TrendingUp, FileText } from "@/lib/icons";
...
export async function AcbWorkspace({ canManageAgenda }: AcbWorkspaceProps) {
  ...
  const closedCompliance = dashboardData.complianceStats.find(
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{dashboardData.criticalObs}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{dashboardData.overdueItems}</div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{dashboardData.recentAudits}</div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{dashboardData.riskMetrics.length}</div>
            <p className="text-xs text-muted-foreground">
          ...
          <CardContent>
            <div className="space-y-3">
              {dashboardData.riskMetrics.map((metric) => (
                <div
                  key={metric.id}
                  className="flex items-center justify-between border-l-4 border-orange-500 pl-4 py-2"
                >
                  <div>
