...
<task type="auto">
  <name>Task 2: Replace hardcoded strings in primary dashboard pages and add print styles</name>
  <files>
    src/app/(dashboard)/dashboard/page.tsx
    src/app/(dashboard)/compliance/page.tsx
    src/app/(dashboard)/audit-plans/page.tsx
  ...
  <action>
    1. **For each of the 5 primary dashboard pages**, replace all hardcoded English strings with translation calls:
  ...
  <verify>
    ...
    - Grep check for remaining hardcoded English UI labels in primary pages:
      `grep -rn '"Dashboard\|"Compliance\|"Audit Plan\|"Finding\|"Report' src/app/\(dashboard\)/dashboard/ src/app/\(dashboard\)/compliance/ src/app/\(dashboard\)/audit-plans/ src/app/\(dashboard\)/findings/ src/app/\(dashboard\)/reports/` should return zero matches for UI label strings (data content matches are acceptable)
    - `pnpm build` succeeds without errors
  </verify>
  <done>5 primary dashboard pages use translation functions for all UI labels, print stylesheet hides nav chrome and formats tables cleanly, reports page has Print button</done>
</task>

<task type="auto">
  <name>Task 3: Replace hardcoded strings in secondary pages</name>
  <files>
    src/app/(dashboard)/settings/page.tsx
    src/app/(dashboard)/auditee/page.tsx
  </files>
  <action>
    ...
    IMPORTANT: Same rules as Task 2 -- only translate UI chrome, not demo data content.
  </action>
  <verify>
    - `pnpm dev` starts without errors
  ...
  <done>All 7 dashboard pages use translation functions for UI labels, zero hardcoded English UI strings remain in component files</done>
</task>
...
8. Grep verification: `grep -rn '"Dashboard\|"Compliance\|"Sign \|"Audit\|"Finding\|"Report\|"Settings\|"Profile' src/app/\(dashboard\)/` returns zero UI label matches
9. Grep verification: `grep -rn 'useTranslations\|getTranslations' src/app/\(dashboard\)/` returns a match for every page file
