...
<task type="auto">
  ...
  <action>
    ...
    2. Create `src/lib/logger.ts` with pino singleton:
       ...
       Configuration:
       - level: "debug" in development, "info" in production (use `process.env.NODE_ENV ?? "development"` directly — do NOT import from env.ts to avoid circular dependency since logger may be used before env validation. This is a known exception to env.ts centralization — documented inline as: `// Known exception: reads NODE_ENV directly to avoid circular dep with env.ts`)
