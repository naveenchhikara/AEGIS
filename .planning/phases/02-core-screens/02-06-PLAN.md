...
files_modified:
  ...
  - src/components/audits/audit-workspace-dialog.tsx
autonomous: false
...
must_haves:
  truths:
    ...
    - Clicking engagement card opens workspace with program linkages (AUDT-06)
  ...
  artifacts:
    ...
    - path: "src/app/(dashboard)/audit-plan/page.tsx"
      ...
      exports: ["default page export"]
      contains: "filter bar, calendar view, engagement cards list, workspace dialog"
    - path: "src/components/audits/audit-workspace-dialog.tsx"
      provides: "Audit workspace detail dialog with program linkages"
      exports: ["AuditWorkspaceDialog"]
      contains: "audit program linkages section, progress tracking"
  key_links:
    ...
    - from: "src/app/(dashboard)/audit-plan/page.tsx"
      ...
      pattern: "auditPlans"
    - from: "src/app/(dashboard)/audit-plan/page.tsx"
      to: "src/components/audits/audit-workspace-dialog.tsx"
      via: "AuditWorkspaceDialog import"
      pattern: "AuditWorkspaceDialog"
    - from: "src/components/audits/audit-workspace-dialog.tsx"
      to: "@/types"
      via: "AuditPlan type import"
      pattern: "AuditPlan"
---
...
<task type="auto">
  ...
  <action>
    ...
    Import: useState, useMemo from 'react', useSearchParams from 'next/navigation', auditPlans from '@/data', AuditFilterBar, AuditCalendar, AuditEngagementCard, AuditWorkspaceDialog from '@/components/audits, Card, CardContent, CardHeader, CardTitle from '@/components/ui/card, type AuditPlan from '@/types
    ...
    State management:
    - selectedAudit state for workspace dialog
    - handleAuditClick: sets selectedAudit to open workspace dialog
    - closeDialog: clears selectedAudit
    ...
    - Click handler opens audit workspace detail dialog
    - Dialog shows audit program linkages (per AUDT-06)

    Workspace dialog:
    - AuditWorkspaceDialog renders when selectedAudit is set
    - onOpenChange clears selectedAudit
    - Displays audit program linkages per AUDT-06
...
<task type="auto">
  <name>Task 4: Create audit workspace detail dialog (AUDT-06)</name>
  <files>src/components/audits/audit-workspace-dialog.tsx</files>
  <action>
    Create src/components/audits/audit-workspace-dialog.tsx:

    Add "use client" directive.

    Import: Dialog, DialogContent, DialogHeader, DialogTitle from @/components/ui/dialog, Badge from @/components/ui/badge, Separator from @/components/ui/separator, Card, CardContent, CardHeader, CardTitle from @/components/ui/card, Calendar, Users, FileText, CheckCircle2 icons from lucide-react, type AuditPlan from '@/types', AuditStatusBadge from '@/lib/status-utils

    Component features:
    - Props: audit, open, onOpenChange
    - Dialog title: audit.name
    - Dialog sections:
      * Header: AuditStatusBadge, formatted date range, team member count
      * Description: audit.description or objective
      * Separator
      * Audit Program Linkages section (AUDT-06):
        - Card with title "Audit Program Linkages"
        - List linked program areas with checkboxes (mock data)
        - Show completion status for each area
      * Progress section: Progress bar with percentage
      * Branches/Departments: Badges list
    - Use Dialog for desktop (ResponsiveDialog optional for Phase 2)

    Export: AuditWorkspaceDialog component
  </action>
  <verify>Component displays audit workspace with program linkages section</verify>
  <done>Audit workspace dialog created with program linkages (AUDT-06)</done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    - Click engagement card: workspace dialog opens
    - Workspace dialog shows audit program linkages (AUDT-06)
...
- Clicking engagement card opens workspace dialog with program linkages (AUDT-06)
- Audit workspace dialog displays program linkages section
...
- AUDT-01 through AUDT-06: Calendar view, engagement cards, type filter, progress bars, workspace with program linkages
