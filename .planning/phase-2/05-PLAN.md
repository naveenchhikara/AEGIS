  ...
  try {
    const result = await db.$transaction(async (tx: any) => {
      ...
      if (
        ...
        complianceItem.status !== "PENDING_BRANCH_RESPONSE"
      ) {
        throw new Error(
          `Cannot submit response for item in ${complianceItem.status} status`,
        );
      }
  ...
  try {
    const result = await db.$transaction(async (tx: any) => {
      ...
      if (complianceItem.status !== "PENDING_ZAC_REVIEW") {
        throw new Error(
          `Cannot review item in ${complianceItem.status} status`,
        );
      }
  ...
  try {
    const result = await db.$transaction(async (tx: any) => {
      ...
      if (complianceItem.status !== "PENDING_ACE") {
        throw new Error(
          `Cannot process item in ${complianceItem.status} status`,
        );
      }
  ...
  try {
    const result = await db.$transaction(async (tx: any) => {
      ...
      if (complianceItem.status !== "PENDING_ACB") {
        throw new Error(
          `Cannot report item in ${complianceItem.status} status`,
        );
      }
    ...
    logger.error(
      { error, complianceItemId, tenantId },
      "Failed to report to ACB",
    );
...
export async function escalateOverdueComplianceItems(tenantId: string) {
  ...
  try {
    ...
    logger.info(
      ...
      "Compliance escalation cron completed",
    );
  } catch (error) {
    logger.error(
      { error, tenantId },
      "Failed to run compliance escalation cron",
    );
    throw error;
  }
}
...
export async function triggerComplianceEscalation() {
  ...
  try {
    ...
    logger.error(
      { error, tenantId },
      "Failed to trigger compliance escalation",
    );
