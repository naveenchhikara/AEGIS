...
must_haves:
  truths:
    ...
    - "Successful push to main triggers Coolify deployment webhook"
...
4. **e2e** job:
   ...
   - `npx playwright install --with-deps chromium` (chromium only, not all browsers)
...
- All 4 validation jobs have NO `needs:` dependency on each other (they run in parallel)
  ...
  Verify all 4 jobs defined: `grep -c "runs-on:" .github/workflows/ci.yml` should return 4
  ...
  <done>CI workflow YAML exists with 4 parallel validation jobs (lint, typecheck, build, e2e), PostgreSQL service container with health checks, pnpm caching, Next.js build cache, and Playwright artifact upload on failure</done>
  </task>
...
- `if` condition ensures deploy only runs on push to main (not PRs)
...
- GET method per Coolify documentation
  ...
  Full YAML validation: `python3 -c "import yaml; yaml.safe_load(open('.github/workflows/ci.yml'))"`
  </verify>
