...
<task type="auto">
  ...
  <action>
    ...
    **Skip-to-content link (Accessibility):**
    Add a visually-hidden skip link at the very top of the layout (before SidebarProvider) that becomes visible on focus:
    ```tsx
    <a href="#main-content" className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-50 focus:rounded-md focus:bg-background focus:px-4 focus:py-2 focus:text-sm focus:font-medium focus:ring-2 focus:ring-ring">
      Skip to main content
    </a>
    ```
    Then add `id="main-content"` on the `<main>` element below.
  ...
  <verify>
    ...
    grep 'Skip to main content' src/app/\(dashboard\)/layout.tsx
    grep 'main-content' src/app/\(dashboard\)/layout.tsx
    pnpm build
    ```
  </verify>
  <done>
