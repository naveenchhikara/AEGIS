...
} from "@/actions/audit-execution/upload-examination-evidence";
...
export function EvidenceUploadPanel({
  ...
  const uploadFile = useCallback(
    async (uploadEntry: UploadEntry) => {
      try {
        ...
        const requestResult = await requestExaminationEvidenceUpload({
          engagementId,
          responseId,
          fileHeader,
        ...
        setEntry({ ...uploadEntry, s3Key });

        // Step 3: Upload to S3 via XHR
    ...
    [engagementId, responseId, onUploadComplete],
  );
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    disabled: entry?.status === "uploading" || entry?.status === "confirming",
  });
