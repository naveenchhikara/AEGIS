      ...
      return {
        ...
        examinationAreas: examinationAreas.map((area) => {
          ...
          return {
            ...
            compliantCount: responses.filter((r) => r.status === "COMPLIANT").length,
            nonCompliantCount: responses.filter((r) => r.status === "NON_COMPLIANT")
              .length,
            partialCount: responses.filter((r) => r.status === "PARTIAL").length,
            naCount: responses.filter((r) => r.status === "NOT_APPLICABLE").length,
          };
        }),
...
<task type="auto">
  ...
  export class XLSXReportGenerator {
    ...
    private async addCoverSheet() {
      ...
      sheet.getCell("B4").value = `${this.data.branch.name} (${this.data.branch.code})`;
      
      sheet.getCell("A5").value = "Audit Period:";
      sheet.getCell("B5").value = this.data.engagement.periodFrom && this.data.engagement.periodTo
    ...
    private async addExecutiveSummarySheet() {
      ...
      const statusGroups = this.groupBy(this.data.complianceItems, (item) => item.status);
    ...
    private async addBranchProfileSheet() {
      ...
      const fields = [
        ...
        ["Audit Frequency (months)", this.data.branch.auditFrequency?.toString() ?? "N/A"],
        ["Last Audit Date", this.data.branch.lastAuditDate ? this.formatDate(this.data.branch.lastAuditDate) : "N/A"],
    ...
    private async addRAMAssessmentSheet() {
      ...
      headerRow.values = ["Parameter Code", "Parameter Name", "Score", "Max Score", "Remarks"];
      ...
      this.data.ramAssessment.scores.forEach((score) => {
        ...
        dataRow.values = [score.paramCode, score.paramName, score.score, "5.00", score.remarks ?? ""];
        row++;
      });
    ...
    private async addObservationsByAreaSheet() {
      ...
      headerRow.values = ["Area Code", "Area Name", "Total Items", "Compliant", "Non-Compliant", "Partial", "N/A"];
      headerRow.font = { bold: true };
      headerRow.fill = {
        type: "pattern",
        pattern: "solid",
    ...
    private async addCashVerificationSheet() {
      ...
      const fields = [
        ...
        ["Verified At", this.data.cashCheck.verifiedAt ? this.formatDate(this.data.cashCheck.verifiedAt) : "N/A"],
      ];
      ...
      if (this.data.cashCheck.atmBalances) {
        ...
        Object.entries(this.data.cashCheck.atmBalances).forEach(([atmId, balance]) => {
    ...
    private async addSmaNpaSheet() {
      ...
      const headerRow = sheet.getRow(3);
      headerRow.values = ["Category", "Account Count", "Total Amount (₹)", "Remarks"];
      headerRow.font = { bold: true };
      headerRow.fill = {
        type: "pattern",
        pattern: "solid",
      ...
      this.data.smaNpaEntries.forEach((entry) => {
        const dataRow = sheet.getRow(row);
        dataRow.values = [entry.category, entry.accountCount, entry.totalAmount, entry.remarks ?? ""];
        row++;
      });
      
      sheet.columns = [{ width: 20 }, { width: 15 }, { width: 20 }, { width: 30 }];
    }
    ...
    private async addComplianceStatusSheet() {
      ...
      this.data.complianceItems.forEach((item) => {
        ...
        dataRow.values = [
          ...
          item.branchResponseDue ? this.formatDate(item.branchResponseDue) : "N/A",
        ];
        row++;
      });
      
      sheet.columns = [{ width: 40 }, { width: 20 }, { width: 12 }, { width: 18 }, { width: 18 }];
    }
...
<task type="auto">
  ...
  export async function generateXLSXReport(engagementId: string) {
    ...
    try {
      ...
      await uploadToS3(s3Key, xlsxBuffer, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
