...
import { Calendar, CheckCircle, Clock, XCircle, ChevronLeft, ChevronRight } from "@/lib/icons";
...
export function BoardReviewCalendar({ meetings, canManage }: BoardReviewCalendarProps) {
  ...
  function getMandatedItemStatus(item: RbiMandatedItem, year: number, month: number) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold tracking-tight">Board Review Calendar</h2>
        ...
        <CardContent>
            ...
            {calendarDays.map((day) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "p-2 text-sm rounded-md border border-transparent transition-colors relative min-h-[60px]",
                    !isSameMonth(day, currentDate) && "text-muted-foreground opacity-50",
                    isToday && "bg-primary/10 border-primary",
                    isSelected && "bg-accent",
                    dayMeetings.length > 0 && "border-blue-300 bg-blue-50 dark:bg-blue-950",
                    "hover:bg-accent"
                  )}
                >
          ...
          {selectedDate && (
            <div className="mt-4 p-4 border rounded-lg bg-muted/50">
              <h4 className="font-medium mb-2">{format(selectedDate, "PPP")}</h4>
        ...
        <CardContent>
          <div className="space-y-3">
            {RBI_MANDATED_ITEMS.map((item, index) => {
              const status = getMandatedItemStatus(item, currentYear, currentMonth);
              const StatusIcon = status && status !== "AS_NEEDED" 
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    status === "COMPLETED" && "bg-green-50 dark:bg-green-950 border-green-300",
                    status === "SCHEDULED" && "bg-blue-50 dark:bg-blue-950 border-blue-300",
                    status === "MISSING" && "bg-red-50 dark:bg-red-950 border-red-300"
                  )}
                >
                  <div className="flex-1">
                    ...
                    {item.months.length > 0 && (
                      <p className="text-xs text-muted-foreground mt-1">
                        Required in: {item.months.map((m) => format(new Date(2024, m - 1, 1), "MMM")).join(", ")}
                      </p>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    {StatusIcon && (
                      <div className="flex items-center gap-1">
                        <StatusIcon.icon className={cn("h-4 w-4", StatusIcon.className)} />
