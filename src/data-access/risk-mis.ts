...
export async function getRiskMisDashboardData(tenantId: string) {
  ...
  const [smaEntries, investmentRecords, housekeepingMetrics, riskRegisters] = await Promise.all([
  ...
  const unreconciledCount = investmentRecords.filter((r) => !r.reconciled).length;

  // Housekeeping risk
  const highAgingCount = housekeepingMetrics.filter((m) => (m.agingDays ?? 0) >= 90).length;
  ...
  const risksByCategory = new Map<string, { count: number; avgResidual: number }>();
  ...
  const operationalRisk = Array.from(risksByCategory.entries()).map(([category, data]) => ({
    category,
    count: data.count,
  ...
  return {
    ...
    investment: {
      totalFaceValue,
      totalMarketValue,
      depreciation: totalFaceValue > 0 ? ((totalFaceValue - totalMarketValue) / totalFaceValue) * 100 : 0,
      unreconciledCount,
    ...
    openRisks: riskRegisters.filter((r) => r.status !== "CLOSED" && r.status !== "MITIGATED").length,
  };
}
