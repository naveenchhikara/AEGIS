...
**Test data strategy:** Tests should create their own observation fixtures as needed for state coverage. Use `test.describe.serial()` to chain tests that depend on prior state (e.g., create Draft observation, transition to Submitted, then test review). Seed data from `prisma/seed.ts` provides the baseline database state (users, roles, tenants, branches) but may not have observations in every lifecycle state. Specifically:

1. **State transition tests (groups 1-3):** Create a fresh observation in the first test of each serial group, then transition through states in subsequent tests
2. **Repeat finding tests (group 7):** Create an initial observation, close it, then create a matching observation to trigger repeat detection
3. **Read-only tests (tags, filters):** Can use existing seed data if available, or create test data in a beforeAll hook
